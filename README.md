...
Prototypes - prop on every func (including constructors), every obj from constructor auto access to defined props/methods on constructor | `Person.prototype` | `Person.prototype.calcAge = function()....` vs not writing method directly in constructor (not present in any obj user of constructor but can still access via prototypal inheritance - doesn't need to be generated for each user) |
