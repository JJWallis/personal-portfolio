...
4. Props - `How do you pass a value from parent to child?` - via props, component takes a value + pass down via props | `What about child to parent?` - pass a func prop (callback) from parent to child via props, child calls with local value passed up to parent as an argument to that func | `What is prop drilling?` - drilling props down a bunch of diff components, grandparent through multiple nested children, passing props via multiple levels
