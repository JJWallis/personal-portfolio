...
Scope Chain = controls how vars organised + accessed by engine | `Lexical scoping` - scoping controlled by placement of funcs + blocks in code (func within another has access to vars in parent) | `scope` - environment in which a var is declared (3 types - `global, func + block`) | scope of a var - entire `region` where a var can be accessed (differences with previous definition of scope generally) | `Global scope` - vars declared outside any func/block, accessible everywhere | `Function/local scope` - each func creates their own + vars decalred here only accessible within it | `Block scope` - created by blocks (everything between {}), same rules as func scope, diff - only let + const vars restricted to block in which created (why they're block scoped - `var` isn't - func scoped so ignores all 'blocks' - 1st img below - belongs to first() not nested 'if' block) | all funcs are block-scoped too in strict mode (funcs decalred inside block only accessible there) | nested scopes - have access to all `outer (parent)` scopes via `looking up` the chain (`variable lookup` - not copied from one scope to another - `never look down`/have access to child scopes) | represented in memory - nested funcs have increasing amount of var environments in their execution contexts, 2nd img below - proof that order of func execution has no impact on scope chain ( diff but related concepts - variables c + b can't be found)
