...
9. Other = `What is a fragment` - container to hold onto different elements, within React can only return one element at a time (multiple - wrapped inside div before but that's why fragment created since doesn't produce anything in the DOM + guarentees only returning one single thing) | `When should you use a Class based component vs a func component` - func components + hooks future of React (all new features using these), one thing class needed for - `error boundaries` since no hooks available to do this yet, React components that catch JS errors anywhere in child component tree - log errors + display fallback UI vs component tree that crashed, errors caught during rendering + lifecycle methods + constructors of entire tree below them | `What is a higher-order component?` - func that takes in a component + returns a new component, re-using component logic, pattern that emerges within React vs official API | `What is a portal?` - newer part of React uncommon (but not in UI libraries), way to render children into a DOM node that exists outside of the DOM heirarchy of the parent component (can live anywhere - common with modals that live separate to rest of tree) | `What are controlled/uncontrolled components?` = uncontrolled - typically input (btn to click with no JS needed to interact with webpage), user manipulated component that user controls + not React, links to virtual DOM vs real DOM, controlled - solution to this problem, React controls state changes in input + can react to those changes (control whether user can interact with it)
