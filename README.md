...
Number conversion + checking - always floating point (even if int - 23 === 23.0) | base 10 system = 0-9 (what we're used to) + nums in JS use base 2 = 0-1 (binary), issue = certain nums hard to represent (0.1 - 0.1 + 0.2 = 0.300...4 but should be 0.3), sometimes rounding auto happens to hide this | 0.1 + 0.2 === 0.3 (false but should be true) | Number obj/func - parsing methods on this (nums from strings), 2nd optional argument - base number of system using (always pass in 10 to avoid some bugs), parseInt`('30', 10)` | Number.isNaN('20' or '20/0') = false (Inifnity + str are regular values - therefore not NaN) - why not perfect | Number.isFinite() = returns true is value passed is finite (neither positive Infinity, negative Infinity, nor NaN), doesn't convert param to number (checks it's of type Number), onyl finite values of the type number return true | Number.isInteger - valid too | (2.7).toFixed(0) = 3 (rounding decimals, returns a str, 2nd arg - amount of decimal places to return) | BigInt = primitive data type, special type introduced in ES2020, nums represented in 64 bits - 64 1/0's to represent a number, only 53 used to store digits themselves - rest for storing position of decimal point + the sign, limit therefore for how big nums can be (Number.MAX_SAFE_INTEGER = largest num JS can safely represent)
